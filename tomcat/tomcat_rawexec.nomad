variables {
  tomcat_download_url = "http://172.34.2.251:2015/apache-tomcat-9.0.64.tar.gz"
  // https://tomcat.apache.org/tomcat-10.0-doc/appdev/sample/
  war_download_url = "http://172.34.2.251:2015/sample.war"
}
...
job "tomcat-10" {
  ...
  group "tomcat" {
    ...
    task "tomcat" {
      ...
      env {
        APP_VERSION   = "0.1"
        CATALINA_HOME = "${NOMAD_TASK_DIR}/apache-tomcat-9.0.64"
...
<Server port="${port.stop}" shutdown="SHUTDOWN">
    ...
    <Service name="Catalina">
        <Connector address="0.0.0.0" port="${port.http}" protocol="HTTP/1.1" connectionTimeout="20000"/>
